<?php
	...
	#Make List
	//Trinity House
	$trinityHouseLighthouses = mysql_query('SELECT * FROM `Lighthouses` WHERE PageType="walestrinityhouse" ORDER BY Name ASC') or die(mysql_error());
	...
	while($i < $numberOfTrinityHouseLighthouses) {
		...
		$lightURL = 'http://www.worldwidelighthouses.com/Lighthouses/Welsh-Lighthouses/Trinity-House-Owned/'.$hyphenName;
	...
	//Privately Owned
	$privateLighthouses = mysql_query('SELECT * FROM `Lighthouses` WHERE PageType="walesprivate" ORDER BY Name ASC');
	...
	while($i < $numberOfPrivateLighthouses) {
		...
		$lightURL = 'http://www.worldwidelighthouses.com/Lighthouses/Welsh-Lighthouses/Privately-Owned/'.$hyphenName;
	...
	$content = '
		<h2 class="greenTitle">Welsh Lighthouses</h2>
		<div class="greenTextbox"><p>Wales main lighthouse authority, like England is Trinity House, you can see their lighthouses in Green. Privately owned scottish lighthouses are in blue.</p></div>
		...
		$pageCode = '<!DOCTYPE HTML>
					<html lang="en-GB">
						<head>	
							...
							<title>Welsh Lighthouses | Worldwide Lighthouses</title>
						</head>
						<body>
							<header> 
	...
	if ($ajax == '1') {
		...
		echo $content.'<script>document.title="Welsh Lighthouses | Worldwide Lighthouses";</script>';
	}
	else {			
